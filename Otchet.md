...
- С помощью команды `hostname` выясним, что при установке ОС машине было задано название `valmerarserver`. 
...
- Как видно из скриншотов, внешний IP-адрес шлюза: 5.228.114.242; а внутренний: 10.0.2.2.
...
- На скриншотах видно, что выставленный вручную статический IP-адрес `10.0.2.45` после перезагрузки остался неизменным.
...
- Поменять hostname ОС от имени пользователя, созданного в пункте часть 2 – создание пользователя (используя sudo).
- Вставить скрин с изменённым hostname в отчёт.

**== Решение ==**

- Разрешим пользователю, созданному в часть 2 – создание пользователя, выполнять команду sudo. Для этого необходимо добавить пользователя `newuser` в группу `sudo`. Воспользуемся командой:
    ```
...
- Поменяем hostname ОС от имени пользователя, созданного в пункте часть 2 – создание пользователя (используя sudo). Для этого необходимо сменить текущего пользователя `valmerar` на ранее созданного `newuser` с помощью команды:
    ```
    su newuser
    ```
...
- VIM. Запишем никнейм `valmerar` , перейдя в режим редактирования нажатием клавиши `i`:
...
- Команда `top` выводит на экран:
  ...
  - загрузка cpu - 0.0 %;
  - загрузка памяти - 174.0 из 3919.5;
  - pid процесса занимающего больше всего памяти - 1;
  - pid процесса, занимающего больше всего процессорного времени - 1194 и 52; 
...
- **htop** :
    ...
    `htop` с процессом `syslog`, найденным, используя поиск (отражается messagebu):
...
- Выяснилось, что:
    - Размер swap: 2345980 килобайта (= 2.24 Гб);
...
- Выполним команду:
    ...
    >Для корневого раздела (/):\
        ...
        - Размер занятого пространства: 5615020;\
        - Размер свободного пространства: 5524632;\
...
- Размеры должны примерно совпадать с полученными в часть 12 использование утилиты du.
...
- На скриншотах видно, размеры папок примерно совпадают с полученными в части 12 задания.
...
- Время последней успешной авторизации: may 25, 19:42:22
